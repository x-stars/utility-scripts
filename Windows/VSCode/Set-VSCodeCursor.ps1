...
$MainCssText = $MainCssText.Replace('.title{cursor:grab}', '.title{cursor:default}')
$MainCssText = $MainCssText.Replace('.pointer{cursor:default}', '.pointer{cursor:pointer}')
$MainCssText = $MainCssText.Replace(
    '.action-menu-item{height:1.8em}', '.action-menu-item{height:1.8em;cursor:default}')
$MainCssText = $MainCssText.Replace(
    '.detected-link-active{cursor:default}', '.detected-link-active{cursor:pointer}')
$MainCssText = $MainCssText.Replace(
    '.button-link:hover{background:transparent}',
    '.button-link:hover{background:transparent;cursor:pointer}')
$MainCssText = $MainCssText.Replace(
    '.codelens-decoration>a:hover{cursor:default}',
    '.codelens-decoration>a:hover{cursor:pointer}')
$MainCssText = $MainCssText.Replace(
    '.goto-definition-link{text-decoration:underline;cursor:default}',
    '.goto-definition-link{text-decoration:underline;cursor:pointer}')
$MainCssText = $MainCssText.Replace(
    '.button-link{padding:0;background:transparent;margin:2px;cursor:default;',
    '.button-link{padding:0;background:transparent;margin:2px;cursor:pointer;')
if (-not $(Test-Path $MainCssBackupPath))
{
...
if (-not $(Test-Path $ReportCssBackupPath))
{
    Move-Item $ReportCssPath $ReportCssBackupPath
}
[System.IO.File]::WriteAllText($ReportCssPath, $ReportCssText, $Encoding)
...
if (-not $(Test-Path $ProcessCssBackupPath))
{
    Move-Item $ProcessCssPath $ProcessCssBackupPath
}
[System.IO.File]::WriteAllText($ProcessCssPath, $ProcessCssText, $Encoding)
